...
export default defineConfig({
    testDir: './tests/e2e',
    /* Keep local runs deterministic against a single dev server. */
    fullyParallel: false,
    /* Fail the build on CI if you accidentally left test.only in the source code. */
    forbidOnly: !!process.env.CI,
    /* Retry on CI only */
    retries: process.env.CI ? 2 : 0,
    /* Use a single worker locally and in CI for stable localhost runs. */
    workers: 1,
    /* Keep local runs clean; emit the HTML report only in CI. */
